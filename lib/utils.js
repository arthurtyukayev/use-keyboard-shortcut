...
export const checkHeldKeysRecursive = (
  ...
  const keyPartOfShortCut = shortcutArray.includes(shortcutKey);

  console.log("shortcutKeyRecursionIndex", shortcutKeyRecursionIndex);
  console.log("heldKeysArray", JSON.stringify(heldKeysArray));
  ...
  if (shortcutIndexOfKey === 0) {
    ...
    if (shortcutKeyRecursionIndex > 0)
      return heldKeysArray.includes(shortcutKey);
    return true;
  }
